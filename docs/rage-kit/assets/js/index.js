...
requestAnimationFrame(() => {
  ...
  }, 80); // shorter duration, more snappy
});
  ...
  button.addEventListener("touchstart", e => {
    e.preventDefault(); // IMPORTANT: stops the click from ALSO firing!
...
function playDrumSound(key, buttonEl) {
  ...
  if (soundFile) {
    ...
    const y = rect.top + rect.height / 2 + window.scrollY + 10; // the +10 fine-tunes spark center
...
const bpm = isTouchDevice ? 110 : 90 + Math.floor(Math.random() * 40);
const interval = isTouchDevice ? 150 : (60 / bpm) * 1000 / 2;
